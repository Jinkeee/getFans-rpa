        ...
        <vertical>
          ...
          >
            ...
            <TextView
              ...
              android:layout_marginRight="5"
              android:text="在搜索栏搜索行业，在用户列表运行，对用户关注，对满足条件的用户执行相应的任务"
              android:textSize="16sp"
              android:layout_alignParentRight="true"
            />
          </RelativeLayout>
...
let data = rpaSettingDataJson.precisionIndustrySetting;
...
ui.id_buttonRun.click(function (view) {
  ...
  let precisionIndustrySetting = {};

  if (followPercent) {
    precisionIndustrySetting.followPercent = followPercent;
  }
  if (dianzanPercent) {
    precisionIndustrySetting.dianzanPercent = dianzanPercent;
  }
  if (commentDianzanPercent) {
    precisionIndustrySetting.commentDianzanPercent = commentDianzanPercent;
  }
  if (commentPercent) {
    precisionIndustrySetting.commentPercent = commentPercent;
  }
  if (candidateComment) {
    precisionIndustrySetting.candidateComment = candidateComment;
  }
  if (minFansCount) {
    precisionIndustrySetting.minFansCount = minFansCount;
  }
  if (maxFansCount) {
    precisionIndustrySetting.maxFansCount = maxFansCount;
  }
  if (videoCount) {
    precisionIndustrySetting.videoCount = videoCount;
  }
  // if(IP) {
  //   accuratePullFansSetting.IP = IP;
  // }
  if (maxRunCount) {
    precisionIndustrySetting.maxRunCount = maxRunCount;
  }
  if (runSpeed) {
    precisionIndustrySetting.runSpeed = runSpeed;
  }

  //写回json
  rpaSettingDataJson.precisionIndustrySetting = precisionIndustrySetting;
  ...
  var precisionIndustry = require("scripts/precisionIndustry");
  // console.log("precisionIndustry", precisionIndustry);
  threads.start(function () {
    precisionIndustry.script();
  });
});
